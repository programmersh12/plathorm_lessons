...
router.post('/start/:quizId', protect, async (req, res) => {
  try {
    ...
    if (inProgressTest) {
      return res.status(400).json({
        success: false,
        message: 'You already have an in-progress test',
        data: { testId: inProgressTest._id }
      });
    }
...
// @access  Private
router.get('/history/:quizId', protect, async (req, res) => {
  try {
    const { quizId } = req.params;
    const studentId = req.user.id;
    ...
    if (req.user.role !== 'admin' && req.user.id !== studentId) {
      return res.status(403).json({
        success: false,
        message: 'Not authorized to view this test history'
      });
    }
...
router.get('/my-tests', protect, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Server error while fetching your tests'
    });
  }
});
