...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // Для /auth/profile не делаем авто-redirect,
      // иначе может происходить лишний переход во время инициализации сессии.
      if (!isProfileRequest) {
...
// Эндпоинты AI-помощника
export const aiAPI = {
  ask: (question) => api.post('/ai', { question }),
};

// Эндпоинты подписок
export const teacherAPI = {
  getTeachers: () => api.get('/subscriptions/teachers'),
  getTeacher: (teacherId) => api.get(`/subscriptions/teachers/${teacherId}`),
  subscribe: (payload) => api.post('/subscriptions/subscribe', payload),
  getMySubscriptions: () => api.get('/subscriptions/my-subscriptions'),
  processPayment: (subscriptionId) => api.post('/subscriptions/payment', { subscriptionId }),
};
