...
const MOCK_COURSES_PRICE = {
  '1': { price: 0, title: 'Основы JavaScript' },
  '2': { price: 49, title: 'Разработка на React' },
  '3': { price: 79, title: 'Разработка мобильных приложений' },
};
...
const CourseDetail = () => {
  ...
  useEffect(() => {
    const mockPrice = MOCK_COURSES_PRICE[courseId] || { price: 49, title: 'Курс' };
    const paidCourses = JSON.parse(localStorage.getItem('paidCourses') || '[]');
    const coursePaid = paidCourses.includes(courseId);
    setIsPaid(mockPrice.price === 0 || coursePaid);
  ...
  return (
    <div style={{ 
      ...
      {(!isPaid && MOCK_COURSES_PRICE[courseId]?.price > 0) && (
        <div style={{
          ...
          <h3 style={{ fontSize: '22px', fontWeight: '700', color: '#92400e', marginBottom: '10px' }}>
            Это платный курс
          </h3>
          <p style={{ color: '#b45309', marginBottom: '20px', fontSize: '16px' }}>
            Приобретите полный доступ к курсу "{MOCK_COURSES_PRICE[courseId]?.title}" за ${MOCK_COURSES_PRICE[courseId]?.price}
          </p>
          <Link
            to={`/payment?courseId=${courseId}`}
            style={{
          ...
          >
            Купить за ${MOCK_COURSES_PRICE[courseId]?.price}
          </Link>
        </div>
      )}
